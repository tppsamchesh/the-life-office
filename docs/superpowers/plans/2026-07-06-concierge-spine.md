...
## Known gap carried forward from the final whole-branch review

Reconciled (recovered) inbound messages are armed with a fresh grace window starting at daemon-boot time (`reconcile.py` calls `on_inbound(current, now, grace_seconds)` using wall-clock `now`, not the message's true Twilio receipt time), because `InboundMessage` (concierge/channels.py) does not carry a timestamp field and `TwilioGateway.list_recent_inbound` does not populate one. Practical effect: a client message that arrived during downtime gets Meg an extra grace window measured from when the daemon comes back up, rather than from when the client actually sent it. This fails safe (Meg gets more time to reply personally, never less) so it was not fixed in Plan 1, but it does mean recovered messages don't get exactly the same grace-window semantics as live ones. Plan 2 or 3 should thread `date_sent` through `InboundMessage` and `list_recent_inbound`, and use it (not wall-clock `now`) when arming the grace timer in `reconcile.py`.

Separately, the startup sweep that requeues stranded `sending`-status messages (`db.reset_stranded_sending()`, added after the final review) trades at-most-once delivery for at-least-once: if the daemon crashes after Twilio already accepted a message but before `mark_sent` recorded that, the sweep requeues it and it will be sent to the client a second time on next boot. This was judged an acceptable tradeoff for informational concierge messages (a rare duplicate "On it!" versus a silently dropped reply), but a real one. `gateway.send` has no idempotency key. If this becomes an issue in practice, or before this system carries any transactional content, add a client-side idempotency token (or persist the Twilio SID immediately after `messages.create` succeeds, before any other write) to close the window.
